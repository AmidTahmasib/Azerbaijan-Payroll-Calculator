    ...
    BEGIN
        ...
        IF p_sector = 'private' THEN
            IF v_gross_salary <= 200 THEN
                v_social_fund_contr := ROUND(v_gross_salary * get_rate('SOCIAL_FUND_RATE', p_sector, v_gross_salary, FALSE), 2);
            ELSE
                v_social_fund_contr := ROUND(get_rate('SOCIAL_FUND_RATE', p_sector, v_gross_salary, TRUE) +
                                             (v_gross_salary - 200) * get_rate('SOCIAL_FUND_RATE', p_sector, v_gross_salary, FALSE), 2);
            END IF;
        ...
        v_total_deductions := ROUND(v_total_deductions + v_social_fund_contr, 2);
        DBMS_OUTPUT.PUT_LINE('DEBUG: Social Fund Contribution: ' || v_social_fund_contr);
